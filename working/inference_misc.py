...
handler_file = FileHandler(filename=f'./logs/inference_misc_{config_filename}_{CFG["model_arch"]}.log')
...
def infer_misc():
    ...
    logger.debug('misc validation loss = {:.5f}'.format(np.mean(val_loss)))
    logger.debug('misc validation accuracy = {:.5f}'.format(np.mean(val_acc)))
